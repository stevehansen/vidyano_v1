...
namespace Vidyano.View
{
    public static class MultiBinding
    {
        ...
        private sealed class MultiBindingHost : DependencyObject
        {
            ...
            public MultiBindingHost(FrameworkElement target, MultiBindingCollection coll)
            {
                ...
                this.coll = coll;
                targetDp = (DependencyProperty)target.GetType().GetField(coll.TargetProperty + "Property").GetValue(null);
                targetProperty = target.GetType().GetProperty(coll.TargetProperty);
            }
