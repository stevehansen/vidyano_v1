...
namespace Vidyano.View
{
    public class Events
    {
        public static readonly DependencyProperty BindingsProperty = DependencyProperty.RegisterAttached("Bindings", typeof(EventBindingCollection), typeof(Events), new PropertyMetadata(null, BindingsCollection_Changed));

        public static EventBindingCollection GetBindings(DependencyObject obj)
        {
            var coll = (EventBindingCollection)obj.GetValue(BindingsProperty);
            if (coll == null)
            {
                coll = new EventBindingCollection();
                SetBindings(obj, coll);
            }
        ...
        public static void AddHandler<T>(DependencyObject d, EventBinding eventBinding, EventInfo runtimeEvent)
        {
            if (eventBinding.handler == null)
            {
                ...
                Func<T, EventRegistrationToken> addMethod = a => (EventRegistrationToken)runtimeEvent.AddMethod.Invoke(d, new object[] { a });
            ...
            WindowsRuntimeMarshal.AddEventHandler((Func<T, EventRegistrationToken>)eventBinding.addMethod, (Action<EventRegistrationToken>)eventBinding.removeMethod, (T)eventBinding.handler);
        }

        public static void RemoveHandler<T>(DependencyObject d, EventBinding eventBinding, EventInfo runtimeEvent)
        {
            if (eventBinding.handler != null)
                WindowsRuntimeMarshal.RemoveEventHandler((Action<EventRegistrationToken>)eventBinding.removeMethod, (T)eventBinding.handler);
        }
    }
